...
@Author: yubin
@Date: 2019-12-03 19:44:31
'''
print("5 fingers classification--linear regression")
...
for test_repitition_id in range(repitition_num):
    ...
    for user_id in range(len(user_name)):
        ...
        for finger_id in range(fingers):
            ...
            train_pred = np.array([1 if pred_>threshold else 0 for pred_ in train_pred])
            train_pred_detail[:,finger_id] = train_pred
            test_pred = clf.predict(test_data.reshape(test_data.shape[0],-1))
            test_pred = np.array([1 if pred_>threshold else 0 for pred_ in test_pred])
